...
import models.resnet2 as resnet
...
parser.add_argument('--gamma', type=float, default=1.0, help='gamma for optimal transport regularization')
parser.add_argument('--K', type=float, default=-1.0, help='the coef of transport cost, -1 meaning K = 1/dim')
...
parser.add_argument('--EMA_startIter', type=int, default=50000, help='start EMA from which G iter')
...
parser.add_argument('--output_root', default=None, help='path to store output samples and checkpoints')
...
if output_root is None:
    print("WARNING: No output_root provided. The output_root is set to current directory.")
    output_root = '.'
if output_dir is None:
    print("WARNING: No output_dir provided. Results will be saved to the outputs folder.")
    output_dir = 'outputs'

output_path = '{0}/{1}'.format(output_root, output_dir)
...
img_path = '{0}/images'.format(output_path)
checkpoint_path = '{0}/checkpoints'.format(output_path)
if not os.path.exists(img_path):
    os.makedirs(img_path)
if not os.path.exists(checkpoint_path):
    os.makedirs(checkpoint_path)

log_fileName = '{0}/{1}_log.txt'.format(output_path, output_dir)
...
def save_images(img_path, real_cpu):
    ...
    vutils.save_image(real_cpu, '{0}/real_samples.png'.format(img_path), nrow=int(batchSize**0.5))
    netG_test.eval()
    with torch.no_grad():
        fake = netG_test(fixed_noise)
    fake.data = fake.cpu().data.mul(0.5).add(0.5)
    vutils.save_image(fake.data, '{0}/fake_samples_{1}.png'.format(img_path, Giter), nrow=int(batchSize**0.5))
    ...
    if Giter % 10 == 0:
        if LAMBDA > 0:
            log_str = '[{:d}/{:d}][{:d}] | WD {:.9f} | real_mean {:.9f} | fake_mean {:.9f} | G_growth {:.9f} | ' \
                      ...
                      'L2LossD {:.9f} | RegLossD {:.9f} | TotalLoss {:.9f} | IS: {:.3f}/{:.3f}'.format(
                epoch, epochs, Giter,
                WD.item(), output_R_mean.item(), output_F_mean.item(), G_growth.item(),
                L2LossD_real.item(), L2LossD_fake.item(),
                          L2LossD.item(), RegLossD.item(), TotalLoss.item(), IS, IS_std)
        else:
            log_str = '[{:d}/{:d}][{:d}] | WD {:.9f} | real_mean {:.9f} | fake_mean {:.9f} | G_growth {:.9f} | ' \
                      'L2LossD_real {:.9f} | L2LossD_fake {:.9f} | ' \
                      'L2LossD {:.9f} | TotalLoss {:.9f} | IS: {:.3f}/{:.3f}'.format(
                epoch, epochs, Giter,
                WD.item(), output_R_mean.item(), output_F_mean.item(), G_growth.item(),
                L2LossD_real.item(), L2LossD_fake.item(),
                          L2LossD.item(), TotalLoss.item(), IS, IS_std)
        print(log_str)
        with open(log_fileName, 'a') as f:
            f.write('{}\n'.format(log_str))
